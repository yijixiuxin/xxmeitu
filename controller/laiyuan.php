...
class laiyuan extends base {
  ...
  public $dbLeimu = '';
    ...
    if('POST' == $_SERVER['REQUEST_METHOD']) {
      ...
      if ($res == false) $this->showMsg('添加来源失败');
      ...
      foreach ($_POST['lurl'] as $k => $lurl) {
        ...
        $this->dbLaiyuan->addSubLei($subData);
      }
  ...
  public function editlaiyuan() {
    if ('POST' == $_SERVER['REQUEST_METHOD']) {
      ...
      if ($res == false) $this->showMsg('修改来源失败');
      $this->dbLaiyuan->delSubLei($where);
      foreach ($_POST['lurl'] as $k => $lurl) {
        $subData['lurl'] = $lurl;
    ...
    if (empty($laiyuanInfo)) $this->showMsg('获取来源信息失败');
    $subleiList = $this->dbLaiyuan->getSubLei('yid='.$yid);
