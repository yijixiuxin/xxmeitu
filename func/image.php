...
function resizeThumbnailImage($thumb,$image,$scale,$fixed=array()){
	...
	if(isset($fixed['width']) && isset($fixed['height'])){
		if ($imagewidth > $fixed['width']) {
			$newImageWidth = $fixed['width'];
			$newImageHeight = ($fixed['width']/$imagewidth) * $imageheight;
		} else if ($imageheight > $fixed['height']) {
			$newImageHeight = $fixed['height'];
			$newImageWidth = ($fixed['height']/$imageheight) * $imagewidth;
		} else {
	...
	switch($imageType) {
		...
		case "image/jpeg":
		case "image/jpg":
			imagejpeg($newImage,$thumb,100*$scale);
			break;
		case "image/png":
		case "image/x-png":
